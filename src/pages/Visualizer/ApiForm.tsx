...
class ApiForm extends React.Component<AppProps, AppState> {
  ...
  handleClick = (e: any) => {
    if (
      this.palletRef &&
      this.palletRef.current &&
      this.palletRef.current.contains(e.target)
    ) {
      return;
    }
    if (
      ...
      this.pickerRef.current.contains(e.target)
    ) {
      return;
    }
    this.setState({
  ...
  onOrientationChange = () => {
    this.setState(
      { horizontalSplit: !this.state.horizontalSplit } as any,
      () => {
      ...
      >
        <form noValidate autoComplete="off" className="input-form">
          ...
          <RadioGroup
            ...
            onChange={(event) =>
              this.handleValueChange(Number(event.target.value))
            }
            row
          >
