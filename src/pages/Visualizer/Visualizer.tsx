...
const VisulizerComponent = ({
  ...
  const onSceneReady = (scene: Scene) => {
    ...
    //https://dl.dropbox.com/s/zvab242zmywk791/SampleScene.babylon?dl=0
    // Import the babylon modal
    //https://dl.dropbox.com/s/pt0hlu4m1ahtmzj/SampleScene.babylon?dl=0
    //https://www.dropbox.com/s/sls9o63pnv2p6ki/SceneGltf.gltf?dl=0
    //https://www.dropbox.com/s/zx5qshqo4bx6sas/SceneObj.obj?dl=0
    //https://www.dropbox.com/s/yzjedpdk42gzf4h/SceneGlb.glb?dl=0
    SceneLoader.ImportMeshAsync(
      "",
      "https://dl.dropbox.com/s/zvab242zmywk791/",
      "SampleScene.babylon",
      scene
    )
      .then(() => {
        ...
        // joint_8!.material = mat_joint6
        // joint_0!.material = mat_joint4
        // Get Gripper Meshes
        gripper_Joint_Inner_Left = scene.getMeshByName(
          "gripper_Joint_Inner_Left"
        );
        gripper_Joint_Outer_Left = scene.getMeshByName(
          "gripper_Joint_Outer_Left"
        );
        gripper_Joint_Inner_Right = scene.getMeshByName(
          "gripper_Joint_Inner_Right"
        );
        gripper_Joint_Outer_Right = scene.getMeshByName(
          "gripper_Joint_Outer_Right"
        );
        ...
        // let distance = Vector3.Distance(joint_1!.position, joint_8!.position)
        // console.log(distance, 'distance')
        // let sphere = MeshBuilder.CreateSphere('box', { diameter: distance }, scene)
    ...
    camera2.angularSensibilityX = 50;
    camera2.angularSensibilityY = 50;
    ...
    new DirectionalLight("DirectionalLight", new Vector3(1, 0, 1), scene);
    new DirectionalLight("DirectionalLight", new Vector3(-1, 0, -1), scene);

    // var pointLight = new PointLight("pointLight", new Vector3(1, 10, 1), scene);
    // // light1.intensity = 0.5;
    // console.log("pointlight", pointLight);
    // pointLight.includeOnlyWithLayerMask;
    var mat = new StandardMaterial("mat", scene);
    var texture = new Texture(
      "https://dl.dropbox.com/s/zlt67vh1fn5h5bh/SampleImage.png",
      scene
    );
    ...
    scene.onPointerUp = function(
      ...
      console.log("camera", camera.alpha, camera.beta);
      ev.preventDefault();
  ...
  useEffect(() => {
    if (scene) {
      if (colorValue === "white") {
        ...
        scene!.clearColor = new Color4(0, 0, 0, 1.0).toLinearSpace();
      }
    }
  }, [colorValue]);
